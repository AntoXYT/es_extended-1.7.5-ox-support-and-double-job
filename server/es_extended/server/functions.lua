...
Core.SavePlayer = function(xPlayer, cb)
	MySQL.prepare("UPDATE `users` SET `accounts` = ?, `job` = ?, `job2` = ?, `job_grade` = ?, `job2_grade` = ?, `group` = ?, `position` = ?, `inventory` = ? WHERE `identifier` = ?", {{
...
Core.SavePlayers = function(cb)
	...
	if count > 0 then
		...
		MySQL.prepare("UPDATE `users` SET `accounts` = ?, `job` = ?, `job2` = ?, `job_grade` = ?, `job2_grade` = ?, `group` = ?, `position` = ?, `inventory` = ? WHERE `identifier` = ?", parameters,
		function(results)
			if results then
...
ESX.GetExtendedPlayers = function(key, val)
	...
	for _, v in pairs(ESX.Players) do
		if key then
			if ((key == 'job' and v.job.name == val) or (key == 'job2' and v.job2.name == val)) or v[key] == val or v.variables[key] == val then
