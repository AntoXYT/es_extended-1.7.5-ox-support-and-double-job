...
CreateThread(function()
	...
	local query = '`accounts`, `job`, `job2`, `job_grade`, `job2_grade`, `group`, `position`, `inventory`, `skin`, `phone_number`'
...
function createESXPlayer(identifier, playerId, data)
	...
	if Config.Multichar and Config.CloneGroup and not identifier:find('char1:') then
		local char1identifier = identifier:gsub('char%d+:', 'char1:')
		local char1 = MySQL.single.await(LoadPlayer, {char1identifier})
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	MySQL.prepare(LoadPlayer, { identifier }, function(result)
		...
		Player.job2 = jobObject2.name
		Player.grade = gradeObject.name
		Player.admin = Core.IsPlayerAdmin(playerId)

		-- Create xPlayer object
		local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.job, userData.job2, userData.playerName, userData.coords, userData.inventory)
