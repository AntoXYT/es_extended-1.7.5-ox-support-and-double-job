...
RegisterNetEvent('esx:playerLoaded', function(xPlayer, isNew, skin)
	...
	if Config.Multichar then
		...
		exports.spawnmanager:spawnPlayer({
			...
			if isNew then
				if skin.sex == 0 then
					TriggerEvent('fivem-appearance:loadDefaultModel', true)
				else
					TriggerEvent('fivem-appearance:loadDefaultModel', false)
				end
			elseif skin then TriggerEvent('fivem-appearance:loadSkin', skin) end
